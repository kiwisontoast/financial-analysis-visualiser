...
def create_date_dropdowns(parent, row, label_text):
    ...
    year = ttk.Combobox(frame, width=5, values=[str(i) for i in range(2000, 2030)])
...
def update_graph():
    ...
    if compare_2_var.get() == 1:
        ...
        if data2 is not None:
            ...
            comparison_result = (
                f"{ticker1} R² ({r2_1:.4f}) vs {ticker2} R² ({r2_2:.4f})\n"
    ...
    if compare_3_var.get() == 1:
        ...
        if data3 is not None:
            ...
            comparison_result = (
                f"{ticker1} R² ({r2_1:.4f}) vs {ticker2} R² ({r2_2:.4f}) vs {ticker3} R² ({r2_3:.4f})\n"
