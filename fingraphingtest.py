...
def toggle_comparison_mode():
    ...
    if compare_3_var.get() == 1:
        # Enable Compare 2 Stocks when Compare 3 Stocks is active
        compare_2_var.set(1)
    if compare_2_var.get() == 1:
        # Show fields for 2 stocks
        ticker2_label.grid()
        ticker2_entry.grid()
    else:
        ticker2_label.grid_remove()
        ticker2_entry.grid_remove()
        # Disable Compare 3 Stocks if Compare 2 Stocks is inactive
        compare_3_var.set(0)
    if compare_3_var.get() == 1:
        # Show fields for 3 stocks
        ticker3_label.grid()
        ticker3_entry.grid()
    ...
    if compare_2_var.get() == 0:
        # Hide result if neither checkbox is active
...
def update_graph():
    ...
    if compare_2_var.get() == 1:
        ...
        if data2 is not None:
            ...
            comparison_result = f"{ticker1} R² ({r2_1:.4f}) vs {
                ticker2} R² ({r2_2:.4f})\n"
            comparison_result += f"Best fit: {
                max([(ticker1, r2_1), (ticker2, r2_2)], key=lambda x: x[1])[0]}"
            comparison_result_label.config(text=comparison_result)
            comparison_result_label.grid()  # Show comparison result label
    ...
    if compare_3_var.get() == 1:
        ...
        if data3 is not None:
            ...
            comparison_result = f"{ticker1} R² ({r2_1:.4f}) vs {ticker2} R² ({r2_2:.4f}) vs {
                ticker3} R² ({r2_3:.4f})\n"
            comparison_result += f"Best fit: {
                max([(ticker1, r2_1), (ticker2, r2_2), (ticker3, r2_3)], key=lambda x: x[1])[0]}"
            comparison_result_label.config(text=comparison_result)
    else:
        if compare_2_var.get() == 0:
            comparison_result_label.config(text="")
            comparison_result_label.grid_remove()  # Hide comparison result label
...
frame.rowconfigure(11, weight=1)  # Adjust row configuration for graph
